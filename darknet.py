...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45):
    ...
    #darkTimeStop = time.perf_counter()
    #print(darkTimeStop - darkTimeStart)
    res = []
    for j in range(num):
...
if __name__ == "__main__":
    ...
    net = load_net(mycfg, myweights, 0)
    meta = load_meta(myCocoData.encode('utf8'))
    #srand(2222222)
    nnp_initialize()
    ...
    #a = detect(net, meta, img2)
    darkTimeStop = time.perf_counter()
    pr.disable()
    pr.print_stats() 
    print(darkTimeStop - darkTimeStart) 
    print(r)
    for foundThing, score, bounds in r:
        x, y, w, h = bounds
        cx1,cy1,cx2,cy2 = convertBack(x, y, w, h)
        cv2.rectangle(img, (cx1, cy1), (cx2, cy2), (255, 0, 0), thickness=2)
